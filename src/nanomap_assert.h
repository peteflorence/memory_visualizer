...
#ifdef NANOMAP_NO_ASSERT
#define NANOMAP_ASSERT(x)
#else
#include <cxxabi.h>
#include <unistd.h>
...
inline void assert_fail(const char *condition, const char *function,
  ...
  fprintf(stderr, "NANOMAP_ASSERT failed: %s in function %s at %s: %i\n",
...
#define NANOMAP_ASSERT(condition) \
  do { \
    if (!(condition)) \
      NANOMAP::assert_fail(#condition, __PRETTY_FUNCTION__, __FILE__, __LINE__); \
  } while (false)
